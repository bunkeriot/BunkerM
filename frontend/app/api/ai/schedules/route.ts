...
async function proxyToCloud(request: NextRequest, path: string, method: string, body?: unknown) {
  const config = readConfig()
  if (!config.api_key || !config.cloud_url) {
    return NextResponse.json({ error: 'BunkerAI Cloud not configured' }, { status: 503 })
  }
  try {
    const resp = await fetch(`${config.cloud_url}${path}`, {
      method,
...
export async function GET() {
  return proxyToCloud(null as never, '/schedules', 'GET')
}

export async function POST(request: NextRequest) {
  const body = await request.json()
  return proxyToCloud(request, '/schedules', 'POST', body)
}
